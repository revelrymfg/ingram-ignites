import { MapPin, Star, Eye, Check } from 'lucide-react'
import { booths, floorIntel } from '../data/mockData'

export default function Booths({ gameState }) {
  const { boothCheckins, checkInBooth } = gameState
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="space-y-3 mb-8">
        {booths.map((booth, i) => {
          const checkedIn = boothCheckins[booth.id]
          return (
            <div
              key={booth.id}
            ...
            >
              ...
              {!checkedIn ? (
                <button
                  onClick={() => checkInBooth(booth.id)}
                  className="bg-surface-border hover:bg-accent/20 text-text-primary text-xs font-semibold uppercase tracking-wider px-4 py-2 min-h-[36px] rounded-lg transition-colors"
                >
                  Check in
                </button>
              ) : (
